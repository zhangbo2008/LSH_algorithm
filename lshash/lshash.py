...
class LSHash(object):
    ...
    :param hash_size:
        ...
        resulting binary hash will be 32-bit long.        # hash值的长度
    :param input_dim:            ###### 特征是多少维度的.
    ...
    def _hash(self, planes, input_point):
        ...
        try:
            ...
            return "".join(['1' if i > 0 else '0' for i in projections])  # 计算为2进制编码!!!!!!!!!!!!!!!!!!!
    ...
    def query(self, query_point, num_results=None, distance_func=None):
        ...
        if distance_func == "hamming":
            ...
            for i, table in enumerate(self.hash_tables):
                binary_hash = self._hash(self.uniform_planes[i], query_point) # 计算查询值的2进制哈希编码
                candidates.update(table.get_list(binary_hash)) # 只抽取哈希值一样的数据. 不一样的直接pass

        # rank candidates by distance function
        candidates = [(ix, d_func(query_point, self._as_np_array(ix)))
                      for ix in candidates]       # 使用距离函数计算 被查询值和数据库所有数据的diff
